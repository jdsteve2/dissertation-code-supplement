...
!---------------------------------------------------------------------
      ...
      double precision  a( 5, 5, ldmx ),  &
     &        b( 5, 5, ldmx ),  &
     &        c( 5, 5, ldmx ),  &
     &        d( 5, 5, ldmx ),  &
     &        u   (5,ldmx/2*2+1,ldmy/2*2+1,ldmz),  &
     &        qs    (ldmx/2*2+1,ldmy/2*2+1,ldmz),  &
     &        rho_i (ldmx/2*2+1,ldmy/2*2+1,ldmz)
      ! js mod:
      double precision c1, c2, c3, c4, c5, dt
      integer ist, iend
      double precision  tx1, tx2
      double precision  ty1, ty2
      double precision  tz1, tz2
...
!$loopy begin
! knl, = lp.parse_fortran(SOURCE, FILENAME)
! RESULT = [knl]
!$loopy end
